...
f = open('urlsCrawledBFSInlinkGraph-new', 'w')
for key,value in url.items():
    f.write("\n%s " %key)
    f.write("%s " %value)
f.close()

f = open('urlsCrawledBFSOutlinkGraph-new', 'w')
for key,value in olink.items():
    f.write("\n%s " %key)
    f.write("%s " %value)
f.close()

with open("urlsCrawledBFSInlinkGraph-encoded.txt", 'wb') as f:
    pickle.dump(url,f)

with open("urlsCrawledBFSOutlinkGraph-encoded.txt", 'wb') as f:
    pickle.dump(olink,f)
